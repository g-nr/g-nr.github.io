...
Most messages start with the same bytes. Whether the message is XML, JSON, FIX, Java Serialization, the first few bytes are predictable.  The start of every encrypted message can end up the same. This make brute force cracking of the key easier because you know what you are looking.

To avoid this we add a 64 bit random number to the start of every message using SecureRandom.  This make brute force attack harder.
