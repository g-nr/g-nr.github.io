...
Peter Lawrey
:hp-tags: Microservice, JDBC, Example

=== A deep dive into a low latency microservice

We loo at a microservice which can run in it's own JVM which can perform JDBC updates and queries via a persisted queue for in bound request and a queue for results.
...
I would consider this a Gateway Service as it inteacts with a system which is outside the microservice model.
...
As in previous posts, we create a component which can be executed without a transport.  This can be unit testsed stand alone, or a series of components without the transport complicating testing and debugging.
...
You can see that every input message creates an output message with the results.  This will be useful later for restarting the service from where it got up to and monitoring it's progress.
...
----
<1> create a thread with a meaningful name. We use an ExecutorService in case we want to something more complex with it later.
...
The performance test write 200K messages as fast as possible and waits for the to all complete.  The first timing is the average latency to write each request, and the second latency is the average time to receive the result.
...
Both Linux and Windows tend to perform well up to 10% of main memory being "dirty" or not written to disk. For example, if you have 256 GB, you can have 25 GB of "dirty" data.  Even so, if the burst faster than the consuming service, but slow enough that the disk subsystem can keep up, your bursts can exceed main memory size.  To put that in context, if you messages are 256 bytes long, the service could be behind by more than one billion messages, and it will not run out of memory, or fail.  The main limitation in this case, is the amount of free disk space you have.  At the time of posting you can buy 1 TB of Enteprise SSD for less than $600, and Samsung is selling 16 TB SSD drives, so there is no sign that storage costs are continuing to fall.
