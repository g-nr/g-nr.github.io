...
*Microservices*- programmes that act as components in a larger network. They work on processes independently, decreasing interdependence and increases cohesion.

*Latency*- gernerally, wasted time while one component is waiting for another to complete its process. http://www.webopedia.com/TERM/L/latency.html["Together, latency and bandwidth define the speed and capacity of a network."]

*Throughput*- the amount of data transferred or processed in a certain amount of time. Ths time is written in terms of throughput, e.g where a road could have a throughput of 10 cars per minute.
